...
library("gridExtra")
# Data input
table <- read.table(file = "hep_lefse_taxa.txt",
    sep = "\t", header = T)
table_heatmap <- read.table(file = "./hep_lefse_taxa_heatmap_samples.txt",
    sep = "\t", header = T)
# Define colors
...
# Pdf
pdf(file = "hep_barplot.pdf", width = 5, height = 6.5)
print(barplot)
dev.off()
pdf(file = "hep_heatmap.pdf", width = 7, height = 6.68)
print(heatmap)
dev.off()
...
#    file = "./hep_lefse_taxa_heatmap_samples.txt",
#    quote = FALSE,
#    row.names = FALSE,
#    sep = "\t")
#
